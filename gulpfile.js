...
gulp.task('watch', function() {                                                 /* задача, которая следит за обновлениями слилистических файлов */
    gulp.watch("src/sass/**/*.+(scss|sass|css)", gulp.parallel('styles'));      /* gulp следит за изменениями sass/scss/css -файлов и при их наличии перезапускает задачу styles */
    ...
    gulp.watch("src/js/**/*.js").on('change', gulp.parallel('scripts'));        /* gulp следит за изменениями js-файлов и при их наличии запускает задачу scripts */
    gulp.watch("src/fonts/**/*").on('all', gulp.parallel('fonts'));             /* gulp следит за любыми действиями, происходящими со шрифтами и при их наличии запускает задачу fonts */
    gulp.watch("src/icons/**/*").on('all', gulp.parallel('icons'));             /* gulp следит за любыми действиями, происходящими с иконками и при их наличии запускает задачу icons */
    gulp.watch("src/img/**/*").on('all', gulp.parallel('images'));              /* gulp следит за любыми действиями, происходящими с картинками и при их наличии запускает задачу images */    
})
...
gulp.task('scripts', function() {   
    return gulp.src("src/js/**/*.js")                                           /* возьмем все скрипты из src/js и скопируем в dist */
        ...
        .pipe(browserSync.stream());                                            /* а после - страница перезагрузится */
}) 
...
gulp.task('default', gulp.parallel('watch', 'server', 'styles', 'html', 'scripts', 'fonts', 'icons', 'images'));  /* по умолчанию, параллельно будут запускаться поманды, что в скобках */